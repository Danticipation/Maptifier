    ...
    public class AndroidPermissionService : IPermissionService
    {
        ...
        private IEnumerator RequestRoutine(string[] permissions, Action<PermissionStatus> onResult)
        {
            ...
            foreach (var p in permissions)
            {
                ...
                callbacks.PermissionDenied += (name) => { completed = true; result = PermissionStatus.Denied; };
                callbacks.PermissionDeniedAndDontAskAgain += (name) => { completed = true; result = PermissionStatus.DeniedAndDontAskAgain; };
